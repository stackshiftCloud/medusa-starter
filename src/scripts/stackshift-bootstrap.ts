...
export default async function stackshiftBootstrap({ container }: ExecArgs) {
  ...
  if (!store) throw new Error("Medusa did not create a default store")
  ...
  }) as unknown as { data: PublishableKey[] }
  ...
  if (!apiKey) {
    ...
    apiKey = result[0] as unknown as PublishableKey
  }
