...
type Probe = { name: string; check: () => Promise<void> }

export async function startStackShiftProviderProbes(
...
export function stackshiftProviderProbes(environment: Environment): Probe[] {
  const probes: Probe[] = []
  if (all(environment, "S3_ENDPOINT", "S3_REGION", "S3_BUCKET", "S3_ACCESS_KEY_ID", "S3_SECRET_ACCESS_KEY")) {
    const client = new StackShiftS3Client({
  ...
  if (all(environment, "STACKSHIFT_MAIL_API_KEY", "STACKSHIFT_MAIL_FROM")) {
    const client = new StackShiftMailClient(
      environment.STACKSHIFT_MAIL_API_KEY!,
      fetch,
      environment.STACKSHIFT_MAIL_URL,
    )
...
function all(environment: Environment, ...keys: string[]): boolean {
